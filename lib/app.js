...
const app = {
  ...
  start (silent = false) {
    ...
    log.info(`Starting up mining-mon-${app.name}`)

    // Configure
    app._readConfigFile('worker.conf', true)
    app._readConfigFile('monitor.conf', true)
    app._readConfigFile('run.conf', false)
    app._templateConfig()
    app._loadMiners()
    app._checkConfig()
    notifyService.notify.config = app.config
    if (!silent) {
      app.printConfig()
    }
  },
  ...
  _readConfigFile (name, optional) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (!optional) {
        app.die(`Config file ${name} not found`)
      }
    }
  },
